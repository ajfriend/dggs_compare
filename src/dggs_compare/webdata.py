...
- globe/{sys}_r{res}_{pos.f32,idx.u32,ar.f32,area.f32,ids.json} — ajglobe's
  native flat-binary polygon format. One globe per system, all at a common cell
  size: the resolution whose cell count is closest to H3's at
  config.GLOBE_H3_RES. Equal-area grids -> matching cell count matches average
  cell area, so this is an area match computed from the closed-form counts (no
  table reads). area.f32 holds each cell's area RATIO to the ideal equal-area
  share (area * N(res) / 4pi, N from the closed forms; globe resolutions are
  enumerated in full, so the ratios average to exactly 1 per globe).
- manifest.json — the data-release tag, per-system web colors/labels, the
  ...
  AR max / area-ratio range (so every globe colors on one comparable scale).
"""
...
def build_globe():
    ...
    Returns {system: res}, the shared AR max, and the shared area-ratio
    (min, max) over those globes' cells."""
    ...
    for s in systems():
        ...
        ratio = cols['area'] * config.CELLS_PER_RES[s](res) / (4.0 * np.pi)
        area_lo = min(area_lo, float(ratio.min()))
        area_hi = max(area_hi, float(ratio.max()))
        pos, starts = [], [0]
        ...
        ratio.astype('<f4').tofile(f'{stem}_area.f32')
...
def build_all():
    ...
    globe_res, globe_max, (area_lo, area_hi) = build_globe()
    ...
    manifest = {
        ...
        'globe_area_min': area_lo,
        'globe_area_max': area_hi,
    ...
    print(f'wrote manifest.json (globe AR range 1..{globe_max:.4f}, '
          f'area ratio {area_lo:.4f}..{area_hi:.4f})')
